...
void    bcopy (void* from, void* to, int nbytes)
{
    memcpy (to, from, nbytes);
}

int     bcmp (void* s1, void* s2, int nbytes)
{
    return memcmp (s1, s2, nbytes);
}

void    bzero (void* b, int nbytes)
{
    memset (b, 0, nbytes);
}
...
#define TIFFERROR(a, b) { \
if (!name_is_set) strcpy(filename, "UNKNOWN");  \
printf("ERROR %s: %s\n", a, b); \
			}
...
#define FatalError(a) {printf("ERROR %s: %s\n", filename, a); exit(-1);}

typedef unsigned char byte;
#define MAX(a, b)      ((a)>(b)?(a):(b))

#include "tiffio.h"
#include "image/vispacktiffioC.h"
#include <stdio.h>
#include <string.h>
...
/* DOn't use C++ comments in C code!!! //static void _TIFFerr();	*/
...
static unsigned name_is_set = 0;

#define GETCWD(x,n) getcwd(x,n)
...
/*******************************************/
{
  ...
  pinfo->type = PIC8;
  strncpy(filename, fname, MAXPATHLEN);    
  name_is_set = 1;
/* use fullname unless it all works out */

  /* open the stream to find out filesize (for info box) */
  fp=fopen(fname,"r");
  if (!fp) {
      TIFFERROR(fname, "LoadTIFF() couldn't open file");
    return 0;
  }
  ...
  if (fname[0] == '/') {
	...
	oldpath[0] = '\0';
	  ...
	  {
	      strncpy(filename, BaseName(fname), MAXPATHLEN);
	      name_is_set = 1;
	  }
    }
  }
     ...
     PICINFO *pinfo;
{
  ...
  pic8 = (byte *) malloc(w*h*(bpp/8));
  if (!pic8) TIFFERROR(filename, "loadPalette() - couldn't malloc 'pic8'");
...
/* change this to support volume images */
/* ********
  ...
  default:
    TIFFERROR(filename, "Sorry, can not handle %d-channel images");
    return (0);
  }
*************/
  ...
  if (!TIFFGetField(tif, TIFFTAG_PHOTOMETRIC, &photometric)) {
    ...
    default:
      TIFFERROR(filename, "Missing needed \"PhotometricInterpretation\" tag");
      return (0);
    }

    TIFFERROR(filename, "No \"PhotometricInterpretation\" tag, assuming %s\n");
  }
     ...
     u_short *r, *g, *b;
{
  ...
  TIFFWARN(filename, "Assuming 8-bit colormap");
  return (8);
}
...
/*set the bytes per pixel */
  bpp = samplesperpixel*MAX((int)(bitspersample/8), 1); 
...
/* not needed if you have the samples per pixel as above */
/*     bpp *= 3; */
...
#define	CVT(x)		((unsigned)((x) * 255) / ((1L<<16)-1))
  ...
  default:
    TIFFERROR(filename, "Unknown photometric tag");
    return (0);
  }
     ...
     u_long h;
{
  ...
  case ORIENTATION_LEFTBOT:	/* XXX */
    TIFFWARN(filename, "using bottom-left orientation");
  ...
  default:
    TIFFWARN(filename, "using top-left orientation");
	...
	int bpp;
{
    ...
    for (row = 0; row < h; row += rowsperstrip) 
	{
		...
		(tif, TIFFComputeStrip(tif, row, 0), buf, 
		 nrow*scanline) 
		< 0 && stoponerr)
		break;
...
static void
put16bitgreytile(cp, pp, Map, w, h, fromskew, toskew)
    register u_short *cp;
    register u_short *pp;
    RGBvalue *Map;
    u_long w, h;
 ...
 */
static void
putRGBcontig8bittile(cp, ppin, Map, w, h, fromskew, toskew)
	register byte *cp;
	void *ppin;
	register RGBvalue *Map;
	u_long w, h;
 ...
 */
static void
putRGBcontig16bittile(cp, ppin, Map, w, h, fromskew, toskew)
	register byte *cp;
	void *ppin;
	register RGBvalue *Map;
	u_long w, h;
 ...
 */
static void
putRGBseparate8bittile(cp, rin, gin, bin, Map, w, h, fromskew, toskew)
	register byte *cp;
	void *rin, *gin, *bin;
	register RGBvalue *Map;
	u_long w, h;
...
{
	register u_char *r = rin, *g = gin, *b = bin;
	if (Map) {
		while (h-- > 0) {
			register u_long x;
			for (x = w; x > 0; x--) {
 ...
 */
static void
putRGBseparate16bittile(cp, rin, gin, bin, Map, w, h, fromskew, toskew)
	register byte *cp;
	void *rin, *gin, *bin;
	register RGBvalue *Map;
	u_long w, h;
	int fromskew, toskew;
{
	register u_short *r = rin, *g = gin, *b = bin;
	register u_long x;
...
#define Code2V(c, RB, RW, CR)     ((((unsigned)(c)-(RB))*(float)(CR))/(float)((RW)-(RB)))
	...
	RGBvalue* Map;
{
	...
	case PHOTOMETRIC_MINISBLACK:
		switch (bitspersample) 
		    {
		      case 16:	put = put16bitgreytile; break;		    
	...
	RGBvalue* Map;
{
 ...
 /*******************************************/
int WriteTIFFRGB(pic,w,h,fname, comp)
byte *pic;
int   w,h;
char *fname;
int comp; /*type of compression */
{
  TIFF *tif;
  FILE *fp;
  ...
  TIFFWriteEncodedStrip(tif, 0, pic, 3*w*h);
...
int WriteTIFFfloat(pic,w,h, channels, fname, comp)
    float *pic;
    int   w,h;
    int   channels;
    char *fname;
...
/*    TIFFSetField(tif, TIFFTAG_DATATYPE, TIFF_FLOAT);  */
    TIFFSetField(tif, TIFFTAG_BITSPERSAMPLE, 32);
    TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, channels);
    TIFFWriteEncodedStrip(tif, 0, (unsigned char *) pic,  sizeof(float)*channels*w*h);
...
int WriteTIFFint(pic,w,h, channels, fname, comp)
    int *pic;
    int   w,h;
    int   channels;
    char *fname;
    int comp; /*type of compression */
{
    ...
    TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, channels);
    TIFFWriteEncodedStrip(tif, 0, (unsigned char *)pic,  sizeof(int)*channels*w*h);
...
int WriteTIFFbyte(pic,w,h, channels, fname, comp)
    byte *pic;
    int   w,h;
    int   channels;
    char *fname;
    int comp; /*type of compression */
{
    ...
    TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, channels);
    TIFFWriteEncodedStrip(tif, 0, pic,  sizeof(byte)*channels*w*h);

    TIFFClose(tif);

    if (fclose(fp) == EOF) {
	printf("tiffwritebyte: somethings wrong unexpected end of file\n");
    }
...
int WriteTIFFshort(pic,w,h, channels, fname, comp)
    short *pic;
    int   w,h;
    int   channels;
    char *fname;
    int comp; /*type of compression */
{
    ...
    TIFFSetField(tif, TIFFTAG_SAMPLESPERPIXEL, channels);
    TIFFWriteEncodedStrip(tif, 0, (unsigned char *)pic, sizeof(short)*channels*w*h);
