...
// $Id: image_type.h,v 1.2 2003-02-26 03:36:45 whitaker Exp $
...
template< >
class VISImageType<schar>
{
  public:
    VISImageType() {}
    VISIm::Type whatAmI() const { return(VISIm::SCHAR); }
    float rangeMin() const {return(SCHAR_MIN);}
    float rangeMax() const {return(SCHAR_MAX);}
};
...
template< >
class VISImageType<unsigned short>
{
  public:
    VISImageType() {}
    VISIm::Type whatAmI() const { return(VISIm::USHORT); }
    float rangeMin() const {return(0);}
    float rangeMax() const {return(USHRT_MAX);}
};
