...
// $Id: image.txx,v 1.3 2005-12-01 03:32:51 miriah Exp $
...
void VISImage<T>::assign(const VISIm& from)
{
    if (!(&from == this))
	{
	    ...
	    else
		{
		    ...
		    VISImageRep<unsigned short> *rep_ushort;
		...
		switch(from.type())
			...
			for (i = 0; i < _channels; i++)
			    {
				rep_ushort = (VISImageRep<unsigned short>*)from.rep(i);
				ref(_rep[i] = new VISImageRep<T>
				    (rep_ushort->width(), rep_ushort->height()));
				::copy(rep_ushort, repRef(i));
			    }
			break;
		      case RGBA:
			for (i = 0; i < _channels; i++)
...
void VISImageRep<T>::evaluate(T (*f)(unsigned int, unsigned int))
{
	...
	for (int j = 0; j < _height; j++)
	    VISImageRep<T>::_buffer[index(i, j)] = f(i, j);// **mdm**
}
...
/* fix _height in image.c */
		...
		for (int j = 0; j < rect_width; j++)
		    VISImageRep<T>::_buffer[index((j + rect_ul_x), (i + rect_ul_y))] = f(j, i);// **mdm**
	      ...
	      unsigned int y_pos)
{
	...
	{	    
	    ...
	    T* buf_out = VISImageRep<T>::bufferRef();// **mdm**
	    const T* buf_in_tmp;
	    T* buf_out_tmp;
