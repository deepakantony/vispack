...
T VISVolume<T>::interp(float x, float y, float z) const
{
    ...
    float z_lo_val = 
	(((float)1.0 - y_in)*
	    (x_in*rep()->itemAt(x_hi, y_lo, z_lo) 
	     + ((float)1.0 - x_in)*rep()->itemAt(x_lo, y_lo, z_lo))
	    + y_in*(x_in*rep()->itemAt(x_hi, y_hi, z_lo) 
		    + ((float)1.0 - x_in)*rep()->itemAt(x_lo, y_hi, z_lo)));

    float z_hi_val = 
	(((float)1.0 - y_in)*
	    (x_in*rep()->itemAt(x_hi, y_lo, z_hi) 
	     + ((float)1.0 - x_in)*rep()->itemAt(x_lo, y_lo, z_hi))
	    + y_in*(x_in*rep()->itemAt(x_hi, y_hi, z_hi) 
		    + ((float)1.0 - x_in)*rep()->itemAt(x_lo, y_hi, z_hi)));
    return((T)(((float)1.0 - z_in)*z_lo_val + z_in*z_hi_val));
}
