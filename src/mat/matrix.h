...
class VISMatrixData{
  ...
  public:
    ...
    VISMatrixData() : _refcnt(0),_data(NULL){_size = 0;}
    VISMatrixData(int size) : _refcnt(0){
	_size = size;
...
//****General functions
  ...
  VISMatrix operator!() const{ 
	if(_cols!=_rows){
	  cout << "VISMatrix operator!(): matrix must be square" << endl;
	  exit(0);}
...
VISMatrix VISIdentity(int size); //return an identity matrix 
VISMatrix homogeneousTransform(VISMatrix &mother, VISVector &vother);

inline istream& operator>> (istream &is, VISMatrix& m){
    m.read(&is); return is;}
inline ostream& operator<<(ostream& os, const VISMatrix& m){
    m.print(&os); return os;}

//VISVector Class
class VISVector : private VISMatrix
{
...
inline VISVector operator*(datatype s,const VISVector v){
    return (v.operator*(s));}
inline VISVector operator+(datatype s,const VISVector v){
    return (v.operator+(s));}
inline VISVector operator-(datatype s,const VISVector v){
...
inline VISVector makept(datatype x,datatype y,datatype z){
    VISVector ans(x,y,z); return ans;}
inline istream& operator>> (istream &is, VISVector& v){
    v.read(&is); return is;}
inline ostream& operator<<(ostream& os, const VISVector& v){
