#ifndef lint
static char rcsid[] = "$Header: /uusoc/res/image/CVS/vispack/packages/tiff/tif_dir.c,v 1.2 2003-02-12 22:47:48 whitaker Exp $";
#endif
...
static
DECLARE3(setShortVISArray, u_short**, wpp, u_short*, wp, long, n)
{
	if (*wpp)
		free((char *)*wpp), *wpp = 0;
	n *= sizeof (u_short);
...
static
DECLARE3(setLongVISArray, u_long**, wpp, u_long*, wp, long, n)
{
	if (*wpp)
		free((char *)*wpp), *wpp = 0;
	n *= sizeof (u_long);
...
static
DECLARE3(setFloatVISArray, float**, wpp, float*, wp, long, n)
{
	if (*wpp)
		free((char *)*wpp), *wpp = 0;
	n *= sizeof (float);
	...
	va_list ap;
{
	...
	case TIFFTAG_COLORMAP:
		...
		setShortVISArray(&td->td_colormap[0], va_arg(ap, u_short *), v);
		setShortVISArray(&td->td_colormap[1], va_arg(ap, u_short *), v);
		setShortVISArray(&td->td_colormap[2], va_arg(ap, u_short *), v);
		break;
	...
	case TIFFTAG_YCBCRCOEFFICIENTS:
		setFloatVISArray(&td->td_ycbcrcoeffs, va_arg(ap, float *), 3);
		break;
	...
	case TIFFTAG_WHITEPOINT:
		setFloatVISArray(&td->td_whitepoint, va_arg(ap, float *), 2);
		break;
	case TIFFTAG_PRIMARYCHROMATICITIES:
		setFloatVISArray(&td->td_primarychromas, va_arg(ap, float *), 6);
		break;
	case TIFFTAG_TRANSFERFUNCTION:
		for (i = 0; i < td->td_samplesperpixel; i++)
		    setShortVISArray(&td->td_transferfunction[i],
			va_arg(ap, u_short *), 1L<<td->td_bitspersample);
		break;
	case TIFFTAG_REFERENCEBLACKWHITE:
		/* XXX should check for null range */
		v = 2 * td->td_samplesperpixel;
		setFloatVISArray(&td->td_refblackwhite, va_arg(ap, float *), v);
		break;
#endif
#ifdef CMYK_SUPPORT
	case TIFFTAG_INKSET:
	...
	case TIFFTAG_IMAGETRANSFORM:
		setFloatVISArray(&td->td_imagetrans, va_arg(ap, float *), 12);
		break;
	default:
		TIFFError(tif->tif_name,
...
#endif
	case TIFFTAG_IMAGETRANSFORM:
		*va_arg(ap, float **) = td->td_imagetrans;
		break;
	default:
