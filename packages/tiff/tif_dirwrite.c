#ifndef lint
static char rcsid[] = "$Header: /uusoc/res/image/CVS/vispack/packages/tiff/tif_dirwrite.c,v 1.2 2003-02-12 22:47:48 whitaker Exp $";
#endif
...
static	TIFFWriteShortVISArray(TIFF *,
	    TIFFDataType, u_short, TIFFDirEntry *, int, u_short *);
static	TIFFWriteLongVISArray(TIFF *,
	    TIFFDataType, u_short, TIFFDirEntry *, int, u_long *);
static	TIFFWriteRationalVISArray(TIFF *,
	    TIFFDataType, u_short, TIFFDirEntry *, int, float *);
static	TIFFWriteFloatVISArray(TIFF *,
	    TIFFDataType, u_short, TIFFDirEntry *, int, float *);
...
static	TIFFWriteShortVISArray();
static	TIFFWriteLongVISArray();
static	TIFFWriteRationalVISArray();
static	TIFFWriteFloatVISArray();
	...
	for (fip = tiffFieldInfo; fip->field_tag; fip++) {
		...
		case FIELD_STRIPOFFSETS:
			...
			if (tag != fip->field_tag)
				continue;
			if (!TIFFWriteLongVISArray(tif, TIFF_LONG, tag, dir,
		...
		case FIELD_REFBLACKWHITE:
			if (!TIFFWriteRationalVISArray(tif, TIFF_RATIONAL,
...
#define	WRITE(x,y)	x(tif, fip->field_type, fip->field_tag, dir, wc, y)
	...
	case TIFF_SSHORT:
		if (wc > 1) {
			...
			if (!WRITE(TIFFWriteShortVISArray, wp))
				return (0);
		} else {
			u_short sv;
	...
	case TIFF_SLONG:
		if (wc > 1) {
			...
			if (!WRITE(TIFFWriteLongVISArray, lp))
				return (0);
		} else {
	...
	case TIFF_SRATIONAL:
		if (wc > 1) {
			...
			if (!WRITE(TIFFWriteRationalVISArray, fp))
				return (0);
		} else {
			float fv;
	...
	case TIFF_FLOAT:
		if (wc > 1) {
			...
			if (wc == (u_short) TIFF_VARIABLE) {
				...
				_TIFFgetfield(td, fip->field_tag, &fp);
			if (!WRITE(TIFFWriteFloatVISArray, fp))
				return (0);
		} else {
			float fv;
			_TIFFgetfield(td, fip->field_tag, &fv);
    ...
    TIFF*, tif, u_short, tag, TIFFDirEntry*, dir)
{
	...
	return (TIFFWriteShortVISArray(tif, TIFF_SHORT, tag, dir, 2, v));
}
 ...
 */
static
DECLARE6(TIFFWriteShortVISArray, TIFF*, tif,
 ...
 */
static
DECLARE6(TIFFWriteLongVISArray, TIFF*, tif,
 ...
 */
static
DECLARE6(TIFFWriteRationalVISArray, TIFF*, tif,
...
static
DECLARE6(TIFFWriteFloatVISArray, TIFF *, tif,
    TIFFDataType, type, u_short, tag, TIFFDirEntry *, dir, int, n, float *, v)
{
	dir->tdir_tag = tag;
	dir->tdir_type = (short)type;
...
DECLARE2(TIFFWriteJPEGQTables, TIFF*, tif, TIFFDirEntry*, dir)
{
	...
	return (TIFFWriteLongVISArray(tif, TIFF_LONG,
    ...
    TIFF*, tif, u_short, tag, TIFFDirEntry*, dir, u_char **, tab)
{
	...
	return (TIFFWriteLongVISArray(tif,
