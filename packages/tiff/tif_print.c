#ifndef lint
static char rcsid[] = "$Header: /uusoc/res/image/CVS/vispack/packages/tiff/tif_print.c,v 1.2 2003-02-12 22:47:49 whitaker Exp $";
#endif
	...
	long flags;
{
	...
	if (TIFFFieldSet(tif,FIELD_IMAGETRANSFORM)) {
		fprintf(fd, "  Image Transform\n");
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 3; j++) {
			    fprintf(fd, " %8.5f", td->td_imagetrans[i*3+j]);
			}
			fprintf(fd, "\n");
		}
	}
