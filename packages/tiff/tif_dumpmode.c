#ifndef lint
static char rcsid[] = "$Header: /uusoc/res/image/CVS/vispack/packages/tiff/tif_dumpmode.c,v 1.2 2003-02-12 22:47:49 whitaker Exp $";
#endif
	...
	u_int s;
{
	...
	while (cc > 0) {
		...
		if (tif->tif_flags & TIFF_SWAB) {
			...
			case 16:
				assert((n & 3) == 0);
				TIFFSwabVISArrayOfShort((u_short *)tif->tif_rawcp,
				    n/2);
				break;
			case 32:
				assert((n & 15) == 0);
				TIFFSwabVISArrayOfLong((u_long *)tif->tif_rawcp,
				    n/4);
				break;
			}
		}
	...
	u_int s;
{
	if (tif->tif_rawcc < cc) {
		TIFFError(tif->tif_name,
		    "DumpModeDecode: Not enough data for scanline %d request %d available %d",
		    tif->tif_row, cc, tif->tif_rawcc);
		return (0);
	}
	/*
	...
	if (tif->tif_flags & TIFF_SWAB) {
		...
		case 16:
			assert((cc & 3) == 0);
			TIFFSwabVISArrayOfShort((u_short *)buf, cc/2);
			break;
		case 32:
			assert((cc & 15) == 0);
			TIFFSwabVISArrayOfLong((u_long *)buf, cc/4);
			break;
		}
	}
	tif->tif_rawcp += cc;
