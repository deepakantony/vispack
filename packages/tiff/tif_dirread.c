#ifndef lint
static char rcsid[] = "$Header: /uusoc/res/image/CVS/vispack/packages/tiff/tif_dirread.c,v 1.2 2003-02-12 22:47:48 whitaker Exp $";
#endif
...
static	TIFFFetchShortVISArray(TIFF *, TIFFDirEntry *, u_short []);
...
static	int TIFFFetchFloatVISArray(TIFF *, TIFFDirEntry *, float *);
...
static	int TIFFFetchFloatVISArray();
	...
	TIFF *tif;
{
	...
	for (fip = tiffFieldInfo, dp = dir, n = dircount; n > 0; n--, dp++) {
		if (tif->tif_flags & TIFF_SWAB) {
			TIFFSwabVISArrayOfShort(&dp->tdir_tag, 2);
			TIFFSwabVISArrayOfLong(&dp->tdir_count, 2);
		}
		/*
	...
	for (dp = dir, n = dircount; n > 0; n--, dp++) {
		...
		case TIFFTAG_PAGENUMBER:
			if (TIFFFetchShortVISArray(tif, dp, td->td_pagenumber))
				TIFFSetFieldBit(tif, FIELD_PAGENUMBER);
			break;
		case TIFFTAG_HALFTONEHINTS:
			if (TIFFFetchShortVISArray(tif, dp, td->td_halftonehints))
		...
		case TIFFTAG_YCBCRSUBSAMPLING:
			if (TIFFFetchShortVISArray(tif, dp, td->td_ycbcrsubsampling))
				TIFFSetFieldBit(tif, FIELD_YCBCRSUBSAMPLING);
			break;
#endif
#ifdef CMYK_SUPPORT
		case TIFFTAG_DOTRANGE:
			if (TIFFFetchShortVISArray(tif, dp, td->td_dotrange))
	...
	char *cp;
{
	...
	if (tif->tif_flags & TIFF_SWAB) {
		...
		case TIFF_SHORT:
		case TIFF_SSHORT:
			TIFFSwabVISArrayOfShort((u_short *)cp, dir->tdir_count);
			break;
		case TIFF_LONG:
		case TIFF_SLONG:
		case TIFF_FLOAT:
			TIFFSwabVISArrayOfLong((u_long *)cp, dir->tdir_count);
			break;
		case TIFF_RATIONAL:
		case TIFF_SRATIONAL:
			TIFFSwabVISArrayOfLong((u_long *)cp, 2*dir->tdir_count);
			break;
		}
	}
	return (cc);
 ...
 */
static
TIFFFetchByteVISArray(tif, dir, v)
	TIFF *tif;
	TIFFDirEntry *dir;
	u_short v[];
{
 ...
 */
static
TIFFFetchShortVISArray(tif, dir, v)
	TIFF *tif;
	TIFFDirEntry *dir;
	u_short v[];
{
 ...
 */
static
TIFFFetchLongVISArray(tif, dir, v)
	TIFF *tif;
	TIFFDirEntry *dir;
	u_long v[];
{
 ...
 */
static
TIFFFetchRationalVISArray(tif, dir, v)
	TIFF *tif;
	TIFFDirEntry *dir;
	float v[];
{
 ...
 */
static
TIFFFetchFloatVISArray(tif, dir, v)
	TIFF *tif;
	TIFFDirEntry *dir;
	float v[];
{
	...
	TIFFDirEntry *dp;
{
	...
	if (dp->tdir_count > 1) {		/* array of values */
		...
		case TIFF_SBYTE:
			...
			ok = cp && TIFFFetchByteVISArray(tif, dp, (u_short *)cp);
			break;
		case TIFF_SHORT:
		case TIFF_SSHORT:
			cp = CheckMalloc(tif,
			    dp->tdir_count * sizeof (u_short), mesg);
			ok = cp && TIFFFetchShortVISArray(tif, dp, (u_short *)cp);
			break;
		case TIFF_LONG:
		case TIFF_SLONG:
			cp = CheckMalloc(tif,
			    dp->tdir_count * sizeof (u_long), mesg);
			ok = cp && TIFFFetchLongVISArray(tif, dp, (u_long *)cp);
			break;
		case TIFF_RATIONAL:
		case TIFF_SRATIONAL:
			cp = CheckMalloc(tif,
			    dp->tdir_count * sizeof (float), mesg);
			ok = cp && TIFFFetchRationalVISArray(tif, dp, (float *)cp);
			break;
		case TIFF_FLOAT:
			cp = CheckMalloc(tif,
			    dp->tdir_count * sizeof (float), mesg);
			ok = cp && TIFFFetchFloatVISArray(tif, dp, (float *)cp);
			break;
		case TIFF_ASCII:
			/*
	...
	u_long **lpp;
{
	...
	if (dir->tdir_type == (int)TIFF_SHORT) {
		...
		if (dp == NULL)
			return (0);
		if (status = TIFFFetchShortVISArray(tif, dir, dp)) {
		...
		status = TIFFFetchLongVISArray(tif, dir, lp);
	return (status);
}
	...
	TIFFDirEntry *dir;
{
	...
	if (ok = (cp && TIFFFetchLongVISArray(tif, dir, (u_long *)cp))) {
		float *fp = (float *)
